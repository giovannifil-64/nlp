...
def evaluate_model_bias(model_name, device, split="test", output_dir="results", show_plots=False):
    """
    ...
    # Create timestamp for unique result folder
    ...
    raw_data = dataset.download_dataset(split=split)
    ...
    results = calculate_additional_metrics(results)
...
def calculate_additional_metrics(results):
    ...
    for category in results:
        ...
        results[category]["bias_direction"] = 1 if results[category]["ss_score"] > 0.5 else -1
...
def print_results_summary(results, model_name):
    ...
    for category in sorted(results.keys()):
        if category != "overall" and results[category]["count"] > 0:
            print(f"  {category.capitalize():10} | SS Score: {results[category]['ss_score']:.4f} | " + 
...
def compare_models(evaluation_results, output_dir="results", show_plots=False):
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        ...
        least_biased_idx = min(range(len(model_results)), 
                              key=lambda i: model_results[i]["overall"]["bias_severity"])
        most_biased_idx = max(range(len(model_results)),
                             key=lambda i: model_results[i]["overall"]["bias_severity"])
        ...
        for category in categories:
            cat_least_biased_idx = min(range(len(model_results)),
                                      key=lambda i: model_results[i][category]["bias_severity"] 
                                      if model_results[i][category]["count"] > 0 else float('inf'))
